...
export const CoursesPage: React.FC = () => {
  ...
  const filteredCourses = useMemo(() => {
    return MOCK_COURSES.filter((course) => {
      ...
      if (filters.tags.length > 0 && !filters.tags.some((tag) => course.tags.includes(tag))) {
        return false
      }
  ...
  const publishedCount = MOCK_COURSES.filter((course) => course.status === 'published').length
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          <Button asChild className="h-12 gap-2 rounded-lg bg-purple-heart-900 px-5 hover:bg-purple-heart-800">
          ...
          <Button
            variant="outline"
            className="h-12 gap-2 rounded-lg border-neutral-200 bg-white px-4 text-neutral-700"
            onClick={() =>
        ...
        <div className="flex items-center justify-end gap-6 border-b border-neutral-200">
          <button
            ...
            className={cn(
              ...
              tab === 'inactive' && 'border-b-2 border-purple-heart-700 text-purple-heart-800',
            )}
          >
          ...
          <button
            ...
            className={cn(
              ...
              tab === 'published' && 'border-b-2 border-purple-heart-700 text-purple-heart-800',
            )}
          >
